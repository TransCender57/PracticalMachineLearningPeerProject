...
cols <- length(NApct[NApct > 0.50])
```

There are `r cols` variables for which the number of NAs exceeds 50%. In
fact, the percentage of NAs in these columns is 98%. A high percentage of
...
Now check the predictors to see if a predictor is a linear combination of
other predictors and remove those predictors.
...
Now that the training data set has been pre-processed, it will be split
into a new training set and a test set.

```{r}
set.seed(1234)
inTrain <- createDataPartition(xformTraining$classe, p=0.7, list=FALSE)
trainSet <- xformTraining[inTrain, ]
testSet <- xformTraining[-inTrain, ]
```

Since we are trying classfy how well the exercise was performed, a tree
...
cfm <- confusionMatrix(pred, testSet$classe)
cfm
```
