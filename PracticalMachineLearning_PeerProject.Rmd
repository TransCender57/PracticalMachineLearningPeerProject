...
There are `r length(NApct[NApct > 0.50])` variables for which the number of
NAs exceeds 50%. In fact, the percentage of NAs in these columns is 98%. A
high percentage of NAs in a predictor provides no information so these
...
Now, check the correlation between the remaining predictors and remove
...
Next, check the remaining predictors to see if a predictor is a linear
combination of other predictors and remove those predictors.
